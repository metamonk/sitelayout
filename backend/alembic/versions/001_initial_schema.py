...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("DRAFT", "ANALYZED", "EXPORTED", name="projectstatus"),
            nullable=True,
        ),
        sa.Column(
            "boundary_geom",
    ...
    op.execute(
        "CREATE INDEX idx_projects_boundary_geom ON projects USING GIST (boundary_geom);"
    )
    op.execute(
        "CREATE INDEX idx_projects_entry_point_geom ON projects USING GIST (entry_point_geom);"
    )
